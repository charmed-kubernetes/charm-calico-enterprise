...
class TigeraCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.on.tigera_relation_changed, self.on_config_changed
        )
    ...
    def tigera_peer_data(self, key):
        ...
        from each tigera unit including self.
    ...
    def preflight_checks(self):
        ...
        try:
            pod_cidr = ipaddress.ip_network(self.model.config["pod_cidr"])
        ...
        if self.model.config["addons"] and not self.model.config["addons_storage_class"]:
            self.unit.status = BlockedStatus("Addons specified but missing addons_storage_class info")
            return False
    ...
    def on_install(self, event):
        ...
        """
        if not self.config(self.config("disable_early_network")):
    ...
    def on_config_changed(self, event):
        ...
        if not self.unit.is_leader():
            ...
            log.info("on_config_changed: detected k8s is up but this unit is not the leader, leaving...")
        ...
        if self.model.config["image_registry"] and self.model.config["image_registry_secret"]:
            image_secret = '\'{"auths":{"' + self.model.config["image_registry"] + '":{"auth":"' + self.model.config["image_registry_secret"] + '","email":""}}}\''
            self.kubectl(
                "apply","secret", "generic", "tigera-pull-secret",
        ...
        self.kubectl(
            "patch", "bgpconfiguration.projectcalico.org", "default", "-p",
