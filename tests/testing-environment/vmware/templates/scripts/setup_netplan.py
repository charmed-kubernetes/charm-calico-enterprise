...
def reconfigure_netplan():
    ...
    netplan["network"]["ethernets"].update(
        {
            "ens192": {
                "routes": [{"to": "default", "via": "10.246.154.134"}],
                "addresses": [
                    [ip["addr_info"][0]["local"] for ip in ip_json if ip["ifname"] == "ens192"][0]
                    + "/24"
                ],
            },
            "ens224": {
                "routes": [{"to": "0.0.0.0/1", "via": "10.246.155.36"}],
                "addresses": [
                    [ip["addr_info"][0]["local"] for ip in ip_json if ip["ifname"] == "ens224"][0]
                    + "/24"
                ],
            },
        }
